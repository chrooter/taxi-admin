					...
					<div class="box txt_wrap">
						<div class="txt pr zoom after">
							...
							<div class="txt_content">
									...
									<tbody>
										<tr>
                                                                                    ...
                                                                                    String type = (String)request.getAttribute("type");
                                                                                     ...
                                                                                     String sort = (type.equals("asc") ? "close_b_div_link": "open_b_div_link");
                                                                                    %>
											<th><div class="th_div b_div">Название<a href="<%=request.getContextPath()%>/ViewCarTypeList?field=NAME&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="th_div b_div">Вместимость<a href="<%=request.getContextPath()%>/ViewCarTypeList?field=CAPACITY&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="th_div b_div">Стоимость за 1 км<a href="<%=request.getContextPath()%>/ViewCarTypeList?field=COST_PER_KM&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="b_div">Имеющиеся автомобили</div></th>
											<th>&nbsp;</th>
										</tr>
										<% int i = 0; %>
										<c:forEach var="carType" items="${requestScope.carTypes}">
										<tr class="<%=(i++ % 2) == 0 ? "tr_1" : "tr_2"%> autotest" id="at${carType.id}">
											<td><c:out value="${carType.name}"/></td>
											<td><c:out value="${carType.capacity}"/></td>
											<td><c:out value="${carType.costPerKm}"/></td>
											<td class="b_div">
												<a href="#" class="b_div_link close_b_div_link" title="Развернуть"></a>
												<div class="box close_b_div">
													<c:if test="${!empty carType.carVOs}">
														<c:forEach var="car" items="${carType.carVOs}">
															<div><a href="<%=request.getContextPath()%>/EditCar?id=${car.id}"><c:out value="${car.model}"/> (<c:out value="${car.govNumber}"/>)</a></div>
														</c:forEach>
													</c:if>
													<c:if test="${empty carType.carVOs}">
														Нет автомобилей данного типа
													</c:if>
												</div>
											</td>
								...
								<c:if test="${empty requestScope.carTypes}">
									<h2>Типов не найдено</h2>
								</c:if>
								<p class="create_test">
								<c:if test="${pageScope.isAdmin}">
									<a href="<%=request.getContextPath()%>/CreateCarType"><img src="<%=request.getContextPath()%>/i/add.png" alt="Добавить"/> <span>Добавить тип</span></a>
                                                                        <a href="<%=request.getContextPath()%>/XMLCarType"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/xml.png" alt="Экпорт"/>Экспорт XML</span></a>
                                                                        <a href="<%=request.getContextPath()%>/importCarType.jsp"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/xml.png" alt="Импорт"/>Импорт XML</span></a>
                                                                        <a href="<%=request.getContextPath()%>/GetLogServlet?entity=CarType"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/list.png" alt="История изменений"/>История изменений</span></a>
								</c:if>
								</p>
