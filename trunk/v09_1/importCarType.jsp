					...
					<div class="box txt_wrap">
						<div class="txt pr zoom after">
							...
							<div class="txt_content">
                                                            <h2>Импорт типов</h2>
                                                            <form action="<%=request.getContextPath()%>/ImportCarType" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
								...
								<c:if test="${!empty requestScope.output}">
                                                                ...
                                                                <p class="create_test">
                                                                    <form>
                                                                        <button formaction="<%=request.getContextPath()%>/CreateOrUpdateCarType" class="btn">Импортировать</button>
                                                                        <button formaction="<%=request.getContextPath()%>/ViewCarTypeList" class="btn">Отменить</button>
                                                                    </form>
								</p>
				...
				$('.open_b_div_link').live('click', function() {
					...
					buttons.addClass('close_b_div_link');
                        ...
                        <c:if test="${requestScope.error == 'wrongXML'}">
                           if (confirm('xml не соответсвует схеме! Загрузить новый?')) { parent.location='<%=request.getContextPath()%>/importCarType.jsp'; }
                            else { parent.location='<%=request.getContextPath()%>/ViewCarTypeList'; }
                        </c:if>
                        <c:if test="${requestScope.error == 'CarType01'}">
