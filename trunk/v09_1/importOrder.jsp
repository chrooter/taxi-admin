					...
					<div class="box txt_wrap">
						<div class="txt pr zoom after">
							<div class="zoom after pr menu1">
								...
								<div class="menu1_notactive fl"><div class="menu1_notactive_wrap"><div class="menu1_notactive_inner"><div class="menu1_notactive_div"><a href="<%=request.getContextPath()%>/ViewCarList">Типы машин</a></div></div></div></div>
							</div>
							<div class="txt_content">
                                                            <h2>Импорт заказов</h2>
                                                            <form action="<%=request.getContextPath()%>/ImportOrder" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
								...
								<c:if test="${!empty requestScope.output}">
                                                                ...
                                                                <p class="create_test">
                                                                    <form>
                                                                        <button formaction="<%=request.getContextPath()%>/CreateOrUpdateOrder" class="btn">Импортировать</button>
                                                                        <button formaction="<%=request.getContextPath()%>/ViewOrderList" class="btn">Отменить</button>
                                                                    </form>
								</p>
				...
				$('.open_b_div_link').live('click', function() {
					...
					buttons.addClass('close_b_div_link');
                         ...
                         <c:if test="${requestScope.error == 'wrongXML'}">
                           if (confirm('xml не соответсвует схеме! Загрузить новый?')) { parent.location='<%=request.getContextPath()%>/importOrder.jsp'; }
                            else { parent.location='<%=request.getContextPath()%>/ViewOrderList'; }
                        </c:if>
                        <c:if test="${requestScope.error == 'Order02'}">
                            if (confirm('Начального адреса не существует! Попробывать еще раз?')) { parent.location='<%=request.getContextPath()%>/importOrder.jsp'; }
                            else { parent.location='<%=request.getContextPath()%>/ViewOrderList'; }
                        </c:if>
