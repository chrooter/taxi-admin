	...
	protected OrderVO getOrder(HttpServletRequest request, int checker) throws CreateException, NamingException, FinderException, RemoveException {
		...
		final String strCarId = request.getParameter("carid");
                ...
                if (carId != 0) {
                    ...
                    CarEntityBeanLocal newcar = home.findByPrimaryKey(carId);
                    carVO = newcar.getCarVO(true,false);
                } else {
                    carVO = null;
                }
		...
		return new OrderVO(id, cost, phone, distance, passengers, timeDone, timeOrd, startPointId, carId, status,carVO);
	}
