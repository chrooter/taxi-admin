							...
							<div class="txt_content">
								...
								<table class="c_table b_table">
									...
									<tbody>
										<tr>
                                                                                    ...
                                                                                    %>
											<th><div class="th_div b_div">Гос. Номер<a href="<%=request.getContextPath()%>/ViewCarList?field=GOV_NUMBER&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="th_div b_div">Модель<a href="<%=request.getContextPath()%>/ViewCarList?field=MODEL&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
                                                                                        <th><div class="th_div b_div">Цвет<a href="<%=request.getContextPath()%>/ViewCarList?field=COLOR&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="th_div b_div">Тип<a href="<%=request.getContextPath()%>/ViewCarList?field=NAME&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
											<th><div class="th_div b_div">Вместимость<a href="<%=request.getContextPath()%>/ViewCarList?field=CAPACITY&type=<%=type%>" class="b_div_link <%=sort%>" title="Сортировка"></a></div></th>
												...
												<div class="box close_b_div">
													<c:if test="${!empty car.currentOrderVOs}">
														<c:forEach var="order" items="${car.currentOrderVOs}">
															<div><a href="<%=request.getContextPath()%>/EditOrder?id=${order.id}">Заказ # <fmt:formatNumber groupingUsed="false" value="${order.id}" minIntegerDigits="4"/></a> <%--(<c:out value="${order.point.address}"/>)--%></div>
														</c:forEach>
													</c:if>
													<c:if test="${empty car.currentOrderVOs}">
														Нет заказов
								...
								<c:if test="${pageScope.isAdmin}">
									<a href="<%=request.getContextPath()%>/CreateCar"><img src="<%=request.getContextPath()%>/i/add.png" alt="Добавить"/> <span>Добавить машину</span></a>
                                                                        <a href="<%=request.getContextPath()%>/XMLCar"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/xml.png" alt="Экпорт"/>Экспорт XML</span></a>
                                                                        <a href="<%=request.getContextPath()%>/importCar.jsp"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/xml.png" alt="Импорт"/>Импорт XML</span></a>
                                                                        <a href="<%=request.getContextPath()%>/GetLogServlet?entity=Car"><span class="xml_link"><img src="<%=request.getContextPath()%>/i/list.png" alt="История изменений"/>История изменений</span></a>
								</c:if>
								</p>
