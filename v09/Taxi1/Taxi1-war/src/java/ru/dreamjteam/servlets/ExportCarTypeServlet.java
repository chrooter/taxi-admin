...
public class ExportCarTypeServlet extends HttpServlet {
    @Override
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
                String filename = "export_cartypes.xml";
                response.setHeader("Content-Disposition","attachment;filename="+filename);
                String mimetype = getServletConfig().getServletContext().getMimeType( filename );
                response.setContentType( (mimetype != null) ? mimetype : "application/octet-stream" );
                
                List<CarTypeVO> carTypes = (List<CarTypeVO>) request.getSession().getAttribute("carTypes");
                try {
                        String h = TaxiBeanEmulator.getXML(carTypes);
