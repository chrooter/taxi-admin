...
public class PointEntityBean implements EntityBean {
        ...
        public void ejbStore() throws EJBException {
            ...
            try {
                ...
                st.setString(1, addr);
                if (nextId != null) st.setInt(2, nextId);
                else st.setNull(2, java.sql.Types.NUMERIC);
                st.setInt(3, id);
                if (st.executeUpdate() < 1)
                    throw new NoSuchEntityException("Row for id " + id + " not found in database");
            } catch (SQLException e) {
        ...
        public Integer generateNextId() throws EJBException {

            PreparedStatement st = null;
                try {
                    openConnection();
                    st = conn.prepareStatement("SELECT POINTS_SEQ.nextval FROM DUAL");
                    ResultSet rs = st.executeQuery();
                    Integer nextId = 0;
                    while (rs.next())
                        nextId = rs.getInt(1);
