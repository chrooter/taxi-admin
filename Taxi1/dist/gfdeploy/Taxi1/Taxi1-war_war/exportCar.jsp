					...
					<div class="box txt_wrap">
						<div class="txt pr zoom after">
							<div class="zoom after pr menu1">
								...
								<div class="menu1_active fl"><div class="menu1_activ_wrap"><div class="menu1_active_inner"><div class="menu1_active_div">Экспорт машин</div></div></div></div>
								<div class="menu1_notactive fl"><div class="menu1_notactive_wrap"><div class="menu1_notactive_inner"><div class="menu1_notactive_div"><a href="<%=request.getContextPath()%>/ViewCarTypeList">Типы машин</a></div></div></div></div>
                                                        ...
                                                        <form action="<%=request.getContextPath()%>/ExportCar" method="POST" accept-charset="UTF-8">
								...
								<table class="c_table b_table">
									...
									<tbody>
										...
										<tr class="<%=(i++ % 2) == 0 ? "tr_1" : "tr_2"%> autotest" id="at${car.id}">
											...
											<td><c:out value="${car.carTypeVO.id}"/></td>
                                                                                        <td class="b_div">
												<a href="#" class="b_div_link close_b_div_link" title="Развернуть"></a>
												<div class="box close_b_div">
													<c:if test="${!empty car.orderVOs}">
														<c:forEach var="order" items="${car.orderVOs}">
															<div>Заказ # <fmt:formatNumber groupingUsed="false" value="${order.id}" minIntegerDigits="4"/></div>
														</c:forEach>
													</c:if>
													<c:if test="${empty car.orderVOs}">
														Нет заказов
