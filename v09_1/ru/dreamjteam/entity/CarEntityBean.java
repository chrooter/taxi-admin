...
public class CarEntityBean implements EntityBean {
	...
	public Integer ejbFindByGovNumber(String govNumber) throws FinderException {
		...
		try {
			openConnection();
			st = conn.prepareStatement("SELECT cars.ID FROM cars WHERE id = (select max(id) from cars)");
		...
		try {
			openConnection();
                        ...
                        if ((!field.equals("NAME")) && (!field.equals("CAPACITY")))
                            sql = "SELECT ID FROM CARS ORDER BY "+field+" "+type;
                        else
		...
		try {
			openConnection();
                        String sql = "SELECT ID FROM CARS WHERE ID=ID" + (carVO.getModel() == null ? "" : " AND MODEL='"+carVO.getModel()+"'")
                                                                       + (carVO.getGovNumber() == null ? "" : " AND GOV_NUMBER='"+carVO.getGovNumber()+"'")
                                                                       + (carVO.getColor() == null ? "" : " AND COLOR='"+carVO.getColor()+"'")
                                                                       + (carVO.getCarTypeId() == null ? "" : " AND TYPE_ID="+carVO.getCarTypeId());
			st = conn.prepareStatement(sql);
			ResultSet resultSet = st.executeQuery();
			Collection<Integer> result = new ArrayList<Integer>();
			while (resultSet.next())
	...
	public CarVO getCarVO(Boolean withDependences,Boolean isExport) throws NamingException, FinderException {
		...
		carVO.setCarTypeVO(carTypeEntity.getCarTypeVO(false,isExport));
                    }
                    return carVO;
            }
		...
		for (Object o : list)
			orders.add(((OrderEntityBeanLocal) o).getOrderVO(false,isExport));
		...
		for (Object o : list2)
			orders2.add(((OrderEntityBeanLocal) o).getOrderVO(false,isExport));
