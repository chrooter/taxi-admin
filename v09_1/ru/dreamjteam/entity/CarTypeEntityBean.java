...
public class CarTypeEntityBean implements EntityBean {
	...
	public Integer ejbFindByLast() throws FinderException {

		PreparedStatement st = null;
		try {
			openConnection();
			st = conn.prepareStatement("SELECT Types.ID FROM types WHERE id = (select max(id) from types)");
	...
	public Collection ejbFindAll() throws FinderException {
		...
		try {
			openConnection();
                        String sql = "SELECT ID FROM TYPES ORDER BY "+field+" "+type;
        ...
        public Collection ejbFindByCarTypeVO(CarTypeVO carTypeVO) throws FinderException {

		PreparedStatement st = null;
		try {
			openConnection();
                        String sql = "SELECT ID FROM TYPES WHERE ID=ID" + (carTypeVO.getCapacity() == null ? "" : " AND CAPACITY="+carTypeVO.getCapacity())
                                                                        + (carTypeVO.getCostPerKm() == null ? "" : " AND COST_PER_KM="+carTypeVO.getCostPerKm())
                                                                        + (carTypeVO.getName() == null ? "" : " AND NAME='"+carTypeVO.getName()+"'");
			st = conn.prepareStatement(sql);
			ResultSet resultSet = st.executeQuery();
			Collection<Integer> result = new ArrayList<Integer>();
			while (resultSet.next())
	...
	public CarTypeVO getCarTypeVO(Boolean withDependences, Boolean isExport) throws NamingException, FinderException {
		...
		for (Object o : list)
			cars.add(((CarEntityBeanLocal) o).getCarVO(false,isExport));
