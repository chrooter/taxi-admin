...
public class OrderEntityBean implements EntityBean {
	...
	public Collection ejbFindByCar(Integer carId) throws FinderException {
		...
		try {
			openConnection();
			st = conn.prepareStatement("SELECT orders.ID FROM ORDERS WHERE id = (select max(id) from orders)");
	...
	public OrderVO getOrderVO(Boolean withDependences, Boolean isExport) throws FinderException, NamingException {
		final OrderVO orderVO = new OrderVO(id, cost, phone, distance, passengers, timeDone, timeOrd, startPoint, carId, status,null);
		if (!withDependences) return orderVO;
                if (carId!=0)
                {
                    ...
                    orderVO.setCar(carEntity.getCarVO(false,isExport));
                }
